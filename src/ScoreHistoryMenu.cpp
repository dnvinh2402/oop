      ...
      titleText(*f, "MATCH HISTORY", 26) 
{
    ...
    // 2. Tải file ảnh historymenu.jpg
    menuBgTexture = new sf::Texture();
    if (!menuBgTexture->loadFromFile("assets/images/historymenu.jpg")) {
    ...
    for (size_t i = 0; i < 5; ++i) {
        ...
        rowText.setFillColor(sf::Color(80, 80, 80)); 
        
        char buf[64];
        if (i < recentScores.size()) {
            std::snprintf(buf, sizeof(buf), "%d. Match %zu:   %d pts", (int)(i + 1), i + 1, recentScores[i]);
        } else {
            std::snprintf(buf, sizeof(buf), "%d. Match %zu:   0 pts", (int)(i + 1), i + 1);
        }
